...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      let resolvedNormalSide = normalSide;
      if (!resolvedNormalSide && parentCode) {
        const parentAccount = await Account_document.findOne({
          code: parentCode,
          companyId,
        })
          .select("normalSide")
          .lean();
        resolvedNormalSide = parentAccount?.normalSide ?? undefined;
      }
      const acc = await Account_document.create({
        ...
        normalSide: resolvedNormalSide, // ✅ ใช้ค่าที่ resolve แล้ว
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      // ถ้าไม่มี parentCode ให้ดึงจาก existingAccount เอง
      if (!resolvedNormalSide) {
        resolvedNormalSide = existingAccount.normalSide;
      }
